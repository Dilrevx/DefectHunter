...
from tensorflow.keras.layers import Dense, Flatten, Input, concatenate, Dropout, Reshape
...
conforencode_emb = ConformerEncoder(
    embedding_dim=768, num_heads=8, feed_forward_dim=1024, num_blocks=12
)(shape_emb1)
conforencode_ast = ConformerEncoder(
    embedding_dim=200, num_heads=8, feed_forward_dim=1024, num_blocks=12
)(sin_ast)
conforencode_cfg = ConformerEncoder(
    embedding_dim=200, num_heads=8, feed_forward_dim=1024, num_blocks=12
)(sin_cfg)
conforencode_dfg = ConformerEncoder(
    embedding_dim=200, num_heads=8, feed_forward_dim=1024, num_blocks=12
)(sin_dfg)
...
model.compile(
    ...
    loss=["sparse_categorical_crossentropy"],  # 8e-7
    metrics=[
        "accuracy",
...
checkpoints = ModelCheckpoint(
    filepath="model/ck/weights.{epoch:04d}.hdf5",
    monitor="val_loss",
    verbose=1,
...
history = model.fit(
    [x_train_emb, x_train_dfg, x_train_cfg, x_train_ast],
    y_train,
...
score = model.evaluate(
    [x_test_emb, x_test_dfg, x_test_cfg, x_test_ast], y_test, verbose=0, batch_size=128
)
print(score)
